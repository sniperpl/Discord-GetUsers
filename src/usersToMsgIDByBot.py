...
def checkAuth():
    global headers, guildId, channelId, msgId, filename, cMsg, imgAmount
    ...
    mid = requests.get(f"https://discord.com/api/{verApi}/channels/{channelId}/messages/{msgId}", headers=headers)
        ...
        while True:
            ...
            for message in messages:                
                ...
                if message["id"] == msgId:
                    ...
                    ######
                    for userId in users:
                        userOnServer = requests.get(f"https://discord.com/api/{verApi}/guilds/{guildId}/members/{userId}", headers=headers)
